...
async function tryOpenDetailControl(
  ...
  if (card.href) {
    ...
    if (await link.isVisible().catch(() => false)) {
      ...
      await clickControl(link, logger);
      return true;
    }
  }
  ...
  if (await corrigendum.isVisible().catch(() => false)) {
    ...
    await clickControl(corrigendum, logger);
    return true;
  }
  ...
  if (await anyCorrText.isVisible().catch(() => false)) {
    ...
    if (await clickable.count().catch(() => 0)) {
      ...
      await clickControl(clickable, logger);
      return true;
    }
  }
  ...
  if (await eyeNamed.isVisible().catch(() => false)) {
    ...
    await clickControl(eyeNamed, logger);
    return true;
  }
  ...
  if (eyeSvg) {
    ...
    await clickControl(eyeSvg, logger);
    return true;
  }
...
async function clickControl(control: Locator, logger: Logger): Promise<void> {
  try {
