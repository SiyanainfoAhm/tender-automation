...
export function TenderAskAiDrawer({ tenderId, tenderTitle, compact = false }: { tenderId: string; tenderTitle: string; compact?: boolean }) {
  ...
  return <>
    ...
    <Sheet open={open} onOpenChange={setOpen}><SheetContent side="right" className="flex w-full flex-col border-l border-slate-200 bg-white p-0 sm:max-w-xl" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}>
